...
static int Gdbmi_tclext_Parse_Cmd(
		 ClientData cdata, 
		 Tcl_Interp *interp, 
		 int objc,
      	 Tcl_Obj * CONST objv[]) {
	...
	if ( objc != 2 ) {
		Tcl_AppendResult ( interp, "bad #arg: ", objv[0], "GDB/MI Output Command", (char*)NULL);
		return TCL_ERROR;
	}
	...
	val = gdbmi_parser_parse_string  ( parser_ptr, mi_output_command, &output_ptr, &parse_failed );
	if ( val == -1 ) {
 ...
 int Gdbmi_tclext_Init(Tcl_Interp *interp) {
	...
	Tcl_CreateObjCommand(
			 ...
			 (ClientData*)parser_ptr, 
			 Gdbmi_tclext_Parse_Destroy);
