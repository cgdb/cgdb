...
static struct tgdb_client_debugger_interfaces {
    ...
    void *(*tgdb_client_create_context) (const char *debugger_path,
            int argc, char **argv,
            const char *config_dir, struct logger * logger);

    int (*tgdb_client_initialize_context) (void *ctx,
            int *debugger_stdin, int *debugger_stdout,
            int *inferior_stdin, int *inferior_stdout);

    int (*tgdb_client_destroy_context) (void *ctx);

    int (*tgdb_client_err_msg) (void *ctx);

    int (*tgdb_client_is_client_ready) (void *ctx);

    int (*tgdb_client_tgdb_ran_command) (void *ctx);

    int (*tgdb_client_prepare_for_command) (void *ctx,
            struct tgdb_command * com);

    int (*tgdb_client_can_tgdb_run_commands) (void *ctx);

    int (*tgdb_client_parse_io) (void *ctx,
    ...
    struct tgdb_list *(*tgdb_client_get_client_commands) (void *ctx);

    int (*tgdb_client_get_filename_pair) (void *ctx, const char *path);

    int (*tgdb_client_get_current_location) (void *ctx, int on_startup);

    int (*tgdb_client_get_inferior_source_files) (void *ctx);
    ...
    char *(*tgdb_client_modify_breakpoint) (void *ctx,
      ...
      pid_t(*tgdb_client_get_debugger_pid) (void *ctx);

    int (*tgdb_client_open_new_tty) (void *ctx,
            int *inferior_stdin, int *inferior_stdout);

    const char *(*tgdb_client_get_tty_name) (void *ctx);

} tgdb_client_debugger_interfaces[] = {
    {
        TGDB_CLIENT_DEBUGGER_GNU_GDB, TGDB_CLIENT_PROTOCOL_GNU_GDB_ANNOTATE_TWO,
    ...
    a2_get_tty_name}, {
        TGDB_CLIENT_DEBUGGER_GNU_GDB, TGDB_CLIENT_PROTOCOL_GNU_GDB_GDBMI,
                /* tgdb_client_create_context */
        ...
        enum tgdb_client_supported_protocols protocol, struct logger *logger)
{
    ...
    if (debugger == TGDB_CLIENT_DEBUGGER_GNU_GDB &&
        ...
        logger_write_pos(tcc->logger, __FILE__, __LINE__,
                "tgdb_client_create_context protocol not recognized");
    }
        ...
        int *inferior_stdin, int *inferior_stdout)
{
    ...
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_initilize_context unimplemented");
        return -1;
    }

    return tcc->tgdb_client_interface->tgdb_client_initialize_context(tcc->
...
int tgdb_client_destroy_context(struct tgdb_client_context *tcc)
{
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_destroy_context unimplemented");
        return -1;
    }

    return tcc->tgdb_client_interface->tgdb_client_destroy_context(tcc->
            tgdb_debugger_context);
}
...
int tgdb_client_is_client_ready(struct tgdb_client_context *tcc)
{
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_is_client_ready unimplemented");
        return -1;
    }

    return tcc->tgdb_client_interface->tgdb_client_is_client_ready(tcc->
            tgdb_debugger_context);
}
...
int tgdb_client_tgdb_ran_command(struct tgdb_client_context *tcc)
{
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_tgdb_ran_command unimplemented");
        return -1;
    }

    return tcc->tgdb_client_interface->tgdb_client_tgdb_ran_command(tcc->
            tgdb_debugger_context);
}
        ...
        struct tgdb_command *com)
{
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_prepare_for_command unimplemented");
        return -1;
    }

    return tcc->tgdb_client_interface->tgdb_client_prepare_for_command(tcc->
            tgdb_debugger_context, com);
}
...
int tgdb_client_can_tgdb_run_commands(struct tgdb_client_context *tcc)
{
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_can_tgdb_run_commands unimplemented");
        return -1;
    }

    return tcc->tgdb_client_interface->tgdb_client_can_tgdb_run_commands(tcc->
            tgdb_debugger_context);
}
        ...
        struct tgdb_list *command_list)
{
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_parse_io error");
        return -1;
    }

    return tcc->tgdb_client_interface->tgdb_client_parse_io(tcc->
            tgdb_debugger_context, input_data, input_data_size, debugger_output,
        ...
        *tcc)
{
    ...
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_get_client_commands error");
        return NULL;
    }

    return tcc->tgdb_client_interface->tgdb_client_get_client_commands(tcc->
            tgdb_debugger_context);
}
        ...
        const char *path)
{
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_get_absolute_path unimplemented");
        return -1;
    }

    return tcc->tgdb_client_interface->tgdb_client_get_filename_pair(tcc->
            tgdb_debugger_context, path);
}
        ...
        int on_startup)
{
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_get_current_location unimplemented");
        return -1;
    }

    return tcc->tgdb_client_interface->tgdb_client_get_current_location(tcc->
            tgdb_debugger_context, on_startup);
}
...
int tgdb_client_get_inferior_source_files(struct tgdb_client_context *tcc)
{
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_get_inferior_source_files unimplemented");
        return -1;
    }

    return tcc->tgdb_client_interface->
            tgdb_client_get_inferior_source_files(tcc->tgdb_debugger_context);
}
        ...
        const char *completion_command)
{
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_completion_callback unimplemented");
        return -1;
    }

    return tcc->tgdb_client_interface->tgdb_client_completion_callback(tcc->
        ...
        enum tgdb_command_type c)
{
    ...
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_return_command unimplemented");
        return NULL;
    }

    return tcc->tgdb_client_interface->tgdb_client_return_command(tcc->
            tgdb_debugger_context, c);
}
        ...
        const char *file, int line, enum tgdb_breakpoint_action b)
{
    ...
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_modify_breakpoint unimplemented");
        return NULL;
    }

    return tcc->tgdb_client_interface->tgdb_client_modify_breakpoint(tcc->
            tgdb_debugger_context, file, line, b);
}
...
pid_t tgdb_client_get_debugger_pid(struct tgdb_client_context * tcc)
{
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_get_debugger_pid unimplemented");
        return -1;
    }

    return tcc->tgdb_client_interface->tgdb_client_get_debugger_pid(tcc->
            tgdb_debugger_context);
}
        ...
        int *inferior_stdin, int *inferior_stdout)
{
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_open_new_tty unimplemented");
        return -1;
    }

    return tcc->tgdb_client_interface->tgdb_client_open_new_tty(tcc->
            tgdb_debugger_context, inferior_stdin, inferior_stdout);
}
...
const char *tgdb_client_get_tty_name(struct tgdb_client_context *tcc)
{
    if (tcc == NULL || tcc->tgdb_client_interface == NULL) {
        logger_write_pos(logger, __FILE__, __LINE__,
                "tgdb_client_get_tty_name unimplemented");
        return NULL;
    }

    return tcc->tgdb_client_interface->tgdb_client_get_tty_name(tcc->
            tgdb_debugger_context);
}
