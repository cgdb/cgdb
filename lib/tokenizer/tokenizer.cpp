...
const char *for_extensions[] = { ".f", "*.for", ".f90", ".f95", "f03" };
...
#define INIT_LEX(_LANG) \
    ...
    if (l == TOKENIZER_LANGUAGE_C) {
        ...
        INIT_LEX(d);
    } else if (l == TOKENIZER_LANGUAGE_FOR) {
        INIT_LEX(for);
        ...
        *file_extension)
{
    ...
    for (i = 0; i < sizeof (for_extensions) / sizeof (char *); i++)
        if (strcasecmp(file_extension, for_extensions[i]) == 0)
            return TOKENIZER_LANGUAGE_FOR;
